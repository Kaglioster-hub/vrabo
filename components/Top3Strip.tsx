...
export default function Top3Strip({ from, to, depart, ret, adults }:{
  ...
  useEffect(()=>{
    ...
    fetch(url).then(r=>r.json()).then(setTop3).catch(()=>setTop3(null));
  ...
  const label = (x:Offer)=> x.label==="cheap"?"Economico":x.label==="fast"?"Veloce":"Migliore";
  return (
    <div className="my-4 mx-auto max-w-5xl rounded-2xl border border-white/10 bg-black/40 p-3 shadow-glow">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        {items.map((o)=>(
          <a key={o.label} href={o.deepLink} target="_blank" rel="nofollow"
             className="block rounded-xl border border-white/10 p-3 hover:bg-white/10">
            <div className="flex items-center justify-between">
              <div className="font-semibold">{label(o)}</div>
              <div className="text-lg">{o.price} {o.currency}</div>
            </div>
            <div className="mt-1 text-sm text-white/80">
              {o.carriers.join(" + ")} · {o.stops===0?"diretto":`${o.stops} scali`} · {o.duration}
            </div>
            <div className="text-xs text-white/60">
              {new Date(o.dep).toLocaleString()} → {new Date(o.arr).toLocaleString()}
            </div>
          </a>
        ))}
      </div>
