...
const norm = (x: Raw): Offer | null => {
  ...
  const dep = x.local_departure || x.utc_departure;
  const arr = x.local_arrival   || x.utc_arrival;
  return {
    label: "best",
    price: x.price,
    currency: x.currency || "EUR",
    carriers,
    stops: Math.max(0, (x.route||[]).length-1),
    duration: fmtDur((x.duration?.total||0)/60),
    dep, arr,
...
export async function GET(req: Request) {
  ...
  const p = new URLSearchParams({
    ...
    date_from: toKiwiDate(depart), date_to: toKiwiDate(depart),
