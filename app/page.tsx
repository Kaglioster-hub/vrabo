...
export default function Home() {
  ...
  const Suggest = () => (
    <div className="text-xs text-white/60">
      ...
      {mode==="stay" && <>Città popolari:&nbsp;{CITIES.map(c=> <button key={c} onClick={()=>setCityTo({label:c,value:c})} className="mx-1 underline hover:text-white">{c}</button>)}</>}
    ...
    <main className="space-y-6">
      <section className="card p-6">
        ...
        {mode==="car" && (
          <div className="grid md:grid-cols-[1fr_1fr] gap-4 mt-6">
            <div className="space-y-3">
              <label className="text-sm text-white/70">Luogo</label>
              <AirportAutocomplete name="to" placeholder="Città o aeroporto di ritiro" value={to} onChange={setTo} onQuery={()=>{}} initialList={initialList} />
              <Suggest/>
            </div>
            <div className="space-y-3">
...
// ---- VRABO inject: state deals Stay/Flight ----
/* inject:start */
// hook deals stay/flight
/* inject:state */'@

# Se l'injection minimale non ti va bene, scriviamo direttamente una pagina pronta (più sicuro)
Set-Content -Path $PageTsx -Encoding UTF8 -Value @'
"use client";
import { useEffect, useState } from "react";
import { PlaneTakeoff, PlaneLanding, Search, Users, Shuffle, LocateFixed, X, BedDouble, Car, Phone, CreditCard } from "lucide-react";
import { Airport, displayAirport, TOP_AIRPORTS } from "@/utils/airports";
...
export default function Home() {
  ...
  // telco/finance/car/stay/flight deals
  const [country, setCountry] = useState<COpt|null>(null);
  const [dealsTelco, setDealsTelco] = useState<Record<string, any>>({});
  const [dealsFin, setDealsFin] = useState<Record<string, any>>({});
  const [dealsCar, setDealsCar] = useState<Record<string, any>>({});
  ...
  const Suggest = () => (
    <div className="text-xs text-white/60">
      {mode==="flight" && <>Suggerimenti:&nbsp;{TOP_AIRPORTS.map(a=> <button key={a} onClick={()=>!from?setFrom({label:a,value:a}):setTo({label:a,value:a})} className="mx-1 underline hover:text-white">{a}</button>)}</>}
      {mode==="stay" && <>Città popolari:&nbsp;{["Roma","Milano","Parigi","Barcellona","Londra","New York","Dubai","Istanbul","Tokyo","Bangkok","Berlino","Amsterdam"].map(c=> <button key={c} onClick={()=>setCityTo({label:c,value:c})} className="mx-1 underline hover:text-white">{c}</button>)}</>}
      {mode==="car" && <>Luoghi comuni:&nbsp;{["FCO","MXP","LIN","LHR","CDG","BCN","MAD","JFK","LAX","FRA","MUC","ATH","NCE","PMI","AGP","CAG","CTA"].map(c=> <button key={c} onClick={()=>setTo({label:c,value:c})} className="mx-1 underline hover:text-white">{c}</button>)}</>}
      {mode==="telco" && <>Paesi eSIM:&nbsp;{["Italia","Spagna","Francia","Germania","USA","Regno Unito","Turchia","Grecia","Thailandia","Giappone"].map(c=> <button key={c} onClick={()=>setCountry({label:c,value:c})} className="mx-1 underline hover:text-white">{c}</button>)}</>}
    </div>
  );
      ...
      <section className="card p-6">
        ...
        {mode==="flight" && (
          <div className="grid md:grid-cols-[1fr_1fr] gap-4 mt-6">
            ...
            <div className="md:col-span-2 pt-2">
              <FlightProviders from={from?.value} to={to?.value} depart={depart?.toISOString().slice(0,10)} ret={ret?.toISOString().slice(0,10)} adults={adults} deals={dealsFlight}/>
            </div>
          </div>
        )}
        ...
        {mode==="stay" && (
          <div className="grid md:grid-cols-[1fr_1fr] gap-4 mt-6">
            ...
            <div className="md:col-span-2 pt-2">
              <HotelProviders city={cityTo?.value} checkin={depart?.toISOString().slice(0,10)} checkout={ret?.toISOString().slice(0,10)} adults={adults} deals={dealsStay}/>
            </div>
          </div>
        )}

        {/* CAR / TELCO / FINANZA: rimangono come prima */}
        {/* ... (codice car/telco/finance già presente, incluso <CarProviders />) ... */}
